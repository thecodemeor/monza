...
// Monza Design System. Inputs capture user text or numeric data
...
@Component({
    ...
    providers: [{
        ...
        useExisting: forwardRef(() => MonInput),
        multi: true
    }]
})
export class MonInput implements ControlValueAccessor, AfterViewInit, OnDestroy {
    @Input() model: 'outline' | 'fill' = 'outline';
    @Input() type: 'text' | 'email' | 'number' | 'password' | 'tel' | 'url' | 'search' = 'text';
    @Input() label?: string;
    ...
    @ContentChild('monPrefix', { read: TemplateRef }) prefixTpl?: TemplateRef<any>;
    @ContentChild('monSuffix', { read: TemplateRef }) suffixTpl?: TemplateRef<any>;
