...
// Monza Design System. Selects provide a controlled way for users
...
import { MonOption } from './option';
import { MonIcon } from '../mon-icon/icon';
...
@Component({
    ...
    providers: [{
        ...
        useExisting: forwardRef(() => MonSelect),
        multi: true
    }]
})
export class MonSelect<T = any> implements ControlValueAccessor, AfterContentInit {
    @Input() label?: string;
    ...
    @ContentChildren(MonOption) optionList!: QueryList<MonOption<T>>;
    ...
    get selectedOption(): MonOption<T> | undefined {
    ...
    get modelClass() {
        return `mon-select mon-select--${this.model}`;
    }
