// ============================================================================
// Monza Accordion Styles
// ============================================================================
// Defines base and variant styles for the Accordion component in the
// Monza Design System. Accordions provide collapsible sections that
...
import { MonIcon } from '../mon-icon/icon'; // adjust path

let nextId = 0;

@Component({
    selector: 'mon-accordion-item',
    standalone: true,
    templateUrl: './accordion-item.html',
    styleUrls: ['./accordion-item.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush,
    imports: [CommonModule, MonIcon]
})
export class MonAccordionItem {
    ...
    @HostBinding('class.mon-accordion-item--expanded')
    get hostExpanded(): boolean {
        return this._expanded;
    }

    @HostBinding('class.mon-accordion-item--disabled')
    get hostDisabled(): boolean {
        return this.disabled;
    }

    // neighbour styling flags
    @HostBinding('class.mon-accordion-item--before-expanded')
    beforeExpanded = false;
    @HostBinding('class.mon-accordion-item--after-expanded')
    afterExpanded = false;

    // DI
    private accordion = inject(MonAccordion, {
        optional: true,
        host: true,
        skipSelf: true
    });
    ...
    panelId = `mon-accordion-panel-${++nextId}`;
